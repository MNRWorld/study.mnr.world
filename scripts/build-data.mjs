...
const PUBLIC_UNI_LIST_PATH = path.join(UNIVERSITIES_DIR, "public-universities.json");
const PRIVATE_UNI_LIST_PATH = path.join(UNIVERSITIES_DIR, "private-universities.json");
...
async function buildUniversityData() {
  ...
  try {
    ...
    console.log(`  - ✅ Finished university data for ${combinedData.length} universities.`);
    return combinedData;
  } catch (error) {
...
async function buildAllData() {
  ...
  try {
    ...
    const otherJsonFiles = allJsonFiles.filter(
      (file) =>
        !file.startsWith(UNIVERSITIES_DIR)
    );
    ...
    const otherDataResults = (await Promise.all(otherDataPromises)).filter(Boolean);
    otherDataResults.forEach(({ key, data }) => {
      allData[key] = data;
    });
    console.log(`  - ✅ Finished processing ${otherDataResults.length} other data files.`);
...
if (shouldWatch) {
  console.log("👀 Watching for changes in all data files (src/lib/data/**/*.json)...");
